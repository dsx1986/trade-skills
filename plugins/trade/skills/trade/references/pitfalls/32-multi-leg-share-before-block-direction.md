...
3. **Reconstruct, don't discard.** Spread legs are informative *as structures*: same session + same expiry + opposite strikes + high multi-leg share usually reassembles into one package. Price the package, not the leg. State explicitly that same-account ownership is an inference — the multi-leg flag proves spread *execution*, not a single book. **In this stack the reassembly is a heuristic, not a lookup** — there is no package-ID endpoint. Massive's option tape gives per-print price / size / exchange / condition codes ([`../massive-data.md`](../massive-data.md) §6), and condition codes *hint* at spread execution but carry no package identity. So the same-session / same-expiry / opposite-strike reconstruction above **is** the method here, and every conclusion drawn from it inherits the inference caveat. When the share estimate is ambiguous, report the block as **activity** and decline to name a direction — that is the whole point of this pitfall.
...
7. **Say what you bounded.** A whole-chain tape pull is expensive, so in practice you scan a subset (top strikes by volume, or a delta band) — which means your counts and premium sums are **bounded by your own query**, not just by a vendor's `limit`. Take direction from the widest aggregate you actually pulled, and state the bound. An unstated bound reads as complete coverage, which is the same defect as an unflagged truncation.
...
- [`../massive-data.md`](../massive-data.md) §6 — why exact package reassembly is **not available** in this stack, so the filter stays a share estimate and must be reported as one
